...
**Geographic map icons for plain HTML/JS — no framework.** Drop `<i data-geoicon="us">` placeholders in your markup, register the icons you use, and `createGeoIcons()` swaps in a live `<svg>`. Tree-shakeable (only registered icons ship), themed entirely with CSS via `currentColor`.
...
Icons are named from **ISO 3166-1 alpha-2** codes (PascalCase): `us` → `Us`, `jp` → `Jp`, `au-mainland` → `AuMainland`. Areas use slug PascalCase: `africa` → `Africa`. The `data-geoicon` key is whatever string you map to the factory (the lowercase code reads naturally: `data-geoicon="us"` → `{ us: Us }`).

```js
// Country icons (base representation):
import { Us, Jp } from '@geoicons/vanilla/countries';

// Areas — continents, regions, blocs, landforms:
import { Africa, Eu } from '@geoicons/vanilla/areas';
```
